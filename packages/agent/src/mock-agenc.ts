...
export class MockAgencAdapter implements AgencPort {
  ...
  private seedDefaultJobs(): void {
    ...
    const specs: Array<{
      ...
      {
        title:
          "Multi-part nexus city research package: intent signing, tax ladder, tokenomics synthesis",
        reward: 30_000_000n, // firm-sized parent job
        caps: 1n,
        firmEligible: true,
      },
      {
    ...
    for (const s of specs) {
      ...
      const listing: JobListing = {
        ...
        claimable,
        source: "mock",
        firmEligible: s.firmEligible ?? s.reward >= 15_000_000n,
      };
  ...
  async postJob(input: {
    ...
    if (input.escrowFromBalance) {
      if (this.balance < input.rewardLamports) {
        throw new Error(
          `Mock AgenC: cannot fund child escrow (need ${input.rewardLamports}, have ${this.balance})`,
        );
      }
      this.balance -= input.rewardLamports;
    }
    const id = randomUUID();
    ...
    const listing: JobListing = {
      ...
      firmEligible: input.rewardLamports >= 15_000_000n,
    };
    this.jobs.set(id, { listing, status: "open" });
    return listing;
  }

  async seedFirmJob(rewardLamports: Lamports = 30_000_000n): Promise<JobListing> {
    return this.postJob({
      title:
        "Firm package: full Nexus City economic brief with multi-section deliverable",
      rewardLamports,
      capabilities: 1n,
      claimable: true,
    });
  ...
  /** Debit living balance (wage transfers, seed capital). */
  debitLamports(amount: Lamports, _reason?: string): void {
    if (this.balance < amount) {
      throw new Error(
        `Mock AgenC: insufficient balance to debit ${amount} (have ${this.balance})`,
      );
    }
    this.balance -= amount;
  }
